...
| ~~ALTA~~ | ~~**Onboarding Bulk Backend** — 3 endpoints faltantes/mismatched~~ — ✅ RESUELTO Phase 168 (2026-06-02): `POST /products/bulk` + `POST /partners/bulk` en master_data_service; `POST /hcm/collaborators/bulk` JSON en hcm_service. `onboarding.service.ts` alineado. |
...
| MEDIA | **MES Shift-End Auto-Logout**: Al terminar el turno, si al colaborador NO se le aprobó tiempo extra (`overtime_approved=False`), ejecutar clock-out automático de todos los `Labor.is_active_labor=True` asociados al `ProductionRun`. Implementar como tarea programada (cron/APScheduler) que corre cada minuto, compara `now()` vs `Shift.end_time + grace_period`, y llama `LaborDensityService.materialize_range()` por cada labor cerrado. Requiere campo `overtime_approved: bool` en `ProductionRun` o en tabla `mes_shift_overtime_approvals`. |
