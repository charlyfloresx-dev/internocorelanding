...
| ~~ALTA~~ | ~~**Landing Onboarding** Completar flujo `http://localhost:4200/onboarding` — Wizard 8 pasos~~ — ✅ RESUELTO Phase 167 (2026-06-02): Angular 19 Signals, CSV drag-drop, `onboarding.service.ts`. |
| ALTA | **Onboarding Bulk Backend** — 3 endpoints faltantes/mismatched. Ver contratos completos en `master_data_service/SERVICE_LOG.md` y `hcm_service/SERVICE_LOG.md`. Detalles: |
| ALTA | → `POST /api/v1/products/bulk` (master_data): **NO EXISTE**. Payload: `{ "products": [{sku, name, category_tag, base_price:{amount,currency}, warehouse_assignments:[{warehouse_code, custom_price, initial_stock}]}] }`. Idempotencia por `(company_id, sku)`. `initial_stock` dispara HTTP a `inventory_service/bulk-load`. |
| ALTA | → `POST /api/v1/partners/bulk` (master_data): **NO EXISTE**. Payload: `{ "partners": [{code, name, type, tax_id, email, phone, address}] }`. Mapeo CSV: `rfc→tax_id`, `city` concatenar a `address`. Idempotencia por `(company_id, code)`. |
| ALTA | → `POST /api/v1/hcm/collaborators/bulk` (hcm): **MISMATCH** (existe `/bulk-upload` CSV; wizard llama path `/bulk` con JSON body). Payload: `{ "collaborators": [{internal_id, first_name, last_name_paternal, department(→id lookup), job_title, rfid_tag(→SHA256+salt), pin_code(→bcrypt), assigned_plant}] }`. |
| ALTA | → **Fix `onboarding.service.ts`**: clave actual es `{ items: rows }` — alinear a `{ products: rows }` / `{ partners: rows }` / `{ collaborators: rows }` al implementar los endpoints. |
