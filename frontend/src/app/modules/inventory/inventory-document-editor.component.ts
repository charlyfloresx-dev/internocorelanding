      ...
      <div class="h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-6 sticky top-0 z-40">
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-3">
            <h1 class="text-xl font-bold text-white">{{ isEdit ? 'Detalle de Documento' : 'Nuevo Documento' }}</h1>
            @if (isLocked) {
              <span class="bg-amber-500/10 text-amber-500 text-[10px] font-bold uppercase px-2 py-1 rounded border border-amber-500/20 flex items-center gap-1">
                <i class="fa-solid fa-lock text-[8px]"></i> Bloqueado
              </span>
            }
          </div>
        </div>
        <div class="flex items-center gap-3">
          ...
          @if (!isLocked) {
            <button (click)="submit()" [disabled]="form.invalid || movements.length === 0" 
               ...
               <i class="fa-solid fa-check"></i> {{ isEdit ? 'Actualizar' : 'Confirmar' }}
            </button>
          }
        </div>
      </div>
            ...
            <form [formGroup]="form" class="space-y-4">
              ...
              <div>
                 <div class="flex justify-between items-end mb-2">
                   <label class="block text-slate-300 text-sm font-bold">Concepto *</label>
                   @if (!isEdit && service.folioPreview()) {
                     <span class="text-[10px] text-sky-400 font-mono">Next Folio: {{ service.folioPreview() }}</span>
                   }
                 </div>
                 <select formControlName="conceptId" (change)="onConceptChange()" class="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:border-sky-500 outline-none transition-colors disabled:opacity-50">
                 ...
                 <!-- Concept Visual Feedback -->
                  @if (selectedConcept(); as concept) {
                    <div [class]="concept.type === 'Entry' 
                       ...
                       <i [class]="concept.type === 'Entry' ? 'fa-solid fa-arrow-right' : 'fa-solid fa-arrow-left'"></i>
                       <span class="font-bold">{{ concept.type === 'Entry' ? 'ENTRADA DE MERCANCÍA' : 'SALIDA DE MERCANCÍA' }}</span>
                    </div>
                  }
              </div>
              ...
              <div>
                 ...
                 <select formControlName="partnershipId" class="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:border-sky-500 outline-none transition-colors disabled:opacity-50">
              ...
              <div class="grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <input type="date" formControlName="deliveryDate" class="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:border-sky-500 outline-none disabled:opacity-50">
                 </div>
                 <div>
                    <label class="block text-slate-300 text-xs font-bold mb-2">Referencia</label>
                    <input type="text" formControlName="reference" class="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:border-sky-500 outline-none disabled:opacity-50" placeholder="Factura, Pedido...">
                 </div>
              </div>
              
              <div>
                 <label class="block text-slate-300 text-xs font-bold mb-2">Descripción</label>
                 <textarea formControlName="description" rows="3" class="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:border-sky-500 outline-none disabled:opacity-50"></textarea>
              </div>
               ...
               <tbody class="divide-y divide-slate-800 bg-slate-900/50">
                 ...
                 @for (row of movements; track $index) {
                   <tr class="group hover:bg-slate-800/50 transition-colors">
                     ...
                     <td class="px-4 py-3">
                        <select [disabled]="isLocked" [ngModel]="row.productId" (ngModelChange)="updateProduct($index, $event)" class="w-full bg-transparent border border-transparent hover:border-slate-700 rounded text-white focus:bg-slate-950 focus:border-sky-500 outline-none disabled:opacity-50">
                     ...
                     <td class="px-4 py-3">
                        <input type="number" [disabled]="isLocked" [(ngModel)]="row.quantity" (ngModelChange)="updateTotal($index)" step="0.0001" class="w-full bg-transparent border border-transparent hover:border-slate-700 rounded text-right text-white font-mono focus:bg-slate-950 focus:border-sky-500 outline-none disabled:opacity-50" min="0">
                     </td>
                     <td class="px-4 py-3 text-slate-500 text-xs">PZA</td>
                     <td class="px-4 py-3">
                        <input type="number" [disabled]="isLocked" [(ngModel)]="row.unitPrice" (ngModelChange)="updateTotal($index)" step="0.0001" class="w-full bg-transparent border border-transparent hover:border-slate-700 rounded text-right text-white font-mono focus:bg-slate-950 focus:border-sky-500 outline-none disabled:opacity-50" min="0">
                     </td>
...
export class InventoryDocumentEditorComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      if (params['id']) {
        ...
        this.loadExistingDocument(params['id']);
      } else {
        this.addRow();
      }
    });
    ...
    effect(() => {
      if (!this.isEdit && !this.form.controls.warehouseId.value && this.service.warehouses().length > 0) {
        this.form.controls.warehouseId.setValue(this.service.warehouses()[0].id);
      }
    });
  }

  async loadExistingDocument(id: string) {
    await this.service.loadDocument(id);
    const doc = this.service.currentDocument();
    if (doc) {
      this.form.patchValue({
        conceptId: doc.conceptId,
        warehouseId: doc.warehouseId,
        partnershipId: doc.partnershipId || '',
        deliveryDate: doc.deliveryDate.slice(0, 10),
        reference: doc.reference,
        description: doc.description
      });
      this.movements = doc.movements.map(m => ({
  ...
  onConceptChange() {
    const conceptId = this.form.value.conceptId;
    if (conceptId) {
      this.service.loadFolioPreview(conceptId);
    }
  }
