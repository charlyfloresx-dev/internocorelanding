### [2026-05-19] Phase 115: GOD MODE Frontend — Sprint 2 Angular + Post-Sprint Hardening

- **`GodModeStore` (`core/stores/god-mode.store.ts`) creado**: Injectable signal store `providedIn: 'root'`. Campos: `token`, `jti`, `expiresAt`, `attempts`. Computados: `isActive` (token ≠ null && Date.now() < expiresAt), `isLocked` (≥3 intentos), `secondsRemaining`. `activate()` lanza `setTimeout(() => clear(), expiresIn * 1000)` — auto-destrucción guaranteed. Sin `localStorage`, sin `sessionStorage`.
- **`godModeInterceptor` (`core/interceptors/god-mode.interceptor.ts`) creado**: `HttpInterceptorFn`. Solo actúa si `store.isActive() === true`. Registrado **al final** del array en `app.config.ts` — decisión crítica: registrarlo primero causaría que `multiTenantInterceptor` sobreescribiera el header `Authorization` con el token de sesión normal, borrando el god-mode token.
- **`SystemControlComponent` (`modules/admin/system-control.component.ts`) creado**: Standalone. UI dual: formulario inactivo (doble confirmación, toggle show/hide de clave, contador attempts) y banner rojo pulsante cuando activo (`animate-ping + animate-pulse`). Countdown reactivo: `toSignal(interval(1000).pipe(map(() => store.secondsRemaining()), takeUntilDestroyed()))`. `closeSession()` llama `DELETE /admin/elevate/{jti}` (revocación server-side) ANTES de `store.clear()`. Master key limpiada del signal inmediatamente tras activación.
- **`ForensicDashboardComponent` extendido (`modules/admin/forensic-dashboard.component.ts`)**: Nueva pestaña "Alertas de Seguridad". Tab switching con `activeTab = signal<'audit' | 'security'>('audit')`. Carga lazy de `GET /api/v1/admin/security-logs?limit=50`. Filas `animate-pulse border-red-300 bg-red-50` para eventos `isRecentAlert` (< 24h). Stat-card GOD MODE en header con contador de eventos. `firstValueFrom` para async/await idiomático.
- **`main-layout.component.ts` — Nav links admin**: Agregados en la sección `@if (navService.isAdmin())`: "Auditoría Forense" → `/admin/forensic` (mat-icon `policy`) y "Consola Emergencia" → `/admin/system-control` (mat-icon `emergency`, colores `text-red-400 hover:bg-red-50`). Ambas rutas ya estaban protegidas por el guard del parent `/admin`.
- **`app.routes.ts` — Ruta system-control**: `{ path: 'system-control', loadComponent: () => import('./modules/admin/system-control.component') }` dentro del children de `admin`.
- **`multi-tenant.interceptor.ts` — stale code removido**: Eliminado bloque `if (auth.isSuperAdmin()) { headers.set('X-Admin-Master-Key', 'GOD_MODE_ACTIVE') }`. Inyectaba el literal hardcodeado en cada request; el nuevo sistema usa el JWT de emergencia via `godModeInterceptor`.
- **TypeScript**: `npx tsc --noEmit` 0 errores en tsconfig.app.json.
- **Status**: ✅ COMPLETED — GOD MODE Angular operativo. Heap: token vive en Signal store (RAM), `clear()` lo nulifica → GC puede reclamarlo inmediatamente. Sin copias en storage persistente.

### [2026-05-18] Phase 112: RBAC Frontend — isReadOnly Fix, Permission Guard, Slug Migration

- **`auth.service.ts` — `isReadOnly` extirpado**: Eliminado `.includes('read')` sobre la lista de permisos. Con slugs granulares como `inventory.stock.read`, el check previo clasificaba a todos los colaboradores como solo-lectura, deshabilitando botones de acción. Ahora depende exclusivamente de `session().readonly` (JWT flag) + estado de suscripción `RESTRICTED` + rol explícito `viewer`.
