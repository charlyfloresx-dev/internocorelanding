...
## 1️⃣ RTR Phase D — Integración en Login Handler

**⏸️ BLOQUEA:**
- Refresh Token Rotation completamente operativo
- Cloud deployment readiness
- User session security

**📋 Qué Falta:**
- Verificar que `select_company_command.py` llama `create_family()` correctamente
- Atomicidad: family se crea con generation=0 en BD
- JWT no se emite si family creation falla

**✅ Cómo Validar:**
```bash
# Ejecutar login flow completo
POST /api/v1/auth/login (T1)
POST /api/v1/auth/select-company (T2)
GET /api/v1/auth/refresh  # debería retornar nuevo token con rotated family

# Verificar en BD:
SELECT * FROM refresh_token_families WHERE company_id = X
  → debe haber una fila con generation=0
```

**📌 Estimado:** 1h  
**🎯 Deadline:** EOD Miércoles
