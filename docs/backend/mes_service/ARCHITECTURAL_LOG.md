...
## AL-010: Desacoplamiento del Monolito Result.cs (CQRS) - 2026-03-05
**Estado:** Aceptado
**Contexto:** La clase legacy `Result.cs` mezclaba responsabilidades de encabezado, paros y KPIs (OEE/LMPU).
**Decisión:** Se descompone en modelos transaccionales (`ProductionRun`, `DowntimeEvent`, `LaborAllocation`) y un Read Model (`RunMetricsSnapshot`).
- **HR Loose Coupling:** Se guardan UUIDs/Strings de líderes/supervisores sin llave foránea al módulo HR para mantener autonomía.
- **Immutable Metrics:** KPIs (LMPU, TakTime, OEE) calculados _solo_ al cierre del turno vía el comando `CloseProductionRun` y almacenados estáticamente para consultas de dashboard O(1).
