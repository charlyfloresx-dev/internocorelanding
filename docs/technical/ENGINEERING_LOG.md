...
## [2026-03-12] Master Data & Audit Engine Pro (Phase 25.3)

### 1. Legacy Consolidation (Item -> Product)
- **Refactor:** Migrated physical attributes (`weight_kg`, `dimensions_json`) and forensic flags (`requires_batch`, `requires_expiration`) from legacy .NET Item analysis to the core `Product` model.
- **Tenancy:** Enforced strict `company_id` isolation across new master entities (`Warehouse`, `MovementConcept`).

### 2. Immutable Auditing (Forensic Ledger)
- **Implementation:** Deployed SQLAlchemy event listeners (`after_insert`, `after_update`, `after_delete`) in `master_data_service` that automatically trigger non-destructive logging of all mutations.
- **Traceability:** Captures state differential (JSONB `old_value`/`new_value`) with full request context (Correlation-ID, IP, User-Agent).

### 3. Container Health & Ops
- **Dockerfile Patch:** Integrated `curl` in `inventory_service` for reliable healthchecks in hybrid environments.
- **Stock Lots:** Scaffolded `stock_lots` table to prepare for lot-level forensic traceability and expiration-based picking logic.

### 4. Initialization (Seeding)
- **Master Data:** Automated loading of ISO UOMs, common conversion factors, and operational movement concepts (Purchase, Sale, Transfer).

---

## [2026-03-15] Mission Control Dashboard Stabilization & High-Fidelity Seeding

### 1. Dashboard Data Integrity (UTC Synchronization)
- **Fix:** Refactored `dashboard_handler.py` and repository logic to use **UTC-aware datetimes** consistently. Naive datetime comparisons were causing empty charts due to timezone mismatches between Python logic and PostgreSQL timestamps.
- **Result:** `movement_series` and `server_time` are now perfectly aligned, ensuring accurate hourly gap filling and chart accuracy.

### 2. High-Fidelity Demo Seeding
- **Seed Optimization:** Updated `inventory_service/scripts/seed.py` to generate a high-density dataset:
  - **4 Warehouses:** Tijuana Central, San Diego Hub, Ensenada Port, Otay Mesa.
  - **Tactical Variations:** Injected specific net changes to match design template contrast (e.g., -34% drop in Otay Mesa, +12% in Ensenada Port).
  - **Critical Alerts:** Dispatched SKU levels (MAT-001, MAT-005) below safety thresholds to trigger real-time dashboard alerts.
- **Cross-Service Sync:** Updated `master-data-service` to provides enriched metadata (product names, warehouse labels) synchronized with the inventory ledger.

### 3. Repository & Traceability
- **Optimization:** Improved `get_dashboard_telemetry` to handle empty datasets and provide precise valuation variations.
- **Compliance:** Enforced strict `X-Trace-ID` propagation through the handler for forensic audit trails in the UI.
