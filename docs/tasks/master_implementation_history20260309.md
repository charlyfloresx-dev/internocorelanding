...
- **Item Variants**:
    - **Implementation**: Created `ItemVariant` model with Brand, MPN, Price, Weight, and Volume.
    - **Data**: Seeded 5 materials with multiple brands (Alcoa, Hydro, 3M, etc.).
- **Movement History**:
    - **Logic**: Generated 20 historical transactions (IN/OUT) within the last 24h.
    - **Alerts**: Forced MAT-001 stock to 12 units (Critical Level).
- **Service Resilience**:
    - **Fix**: Resolved `uuid.UUID` type conflicts in `Movement` and `Concept` models.
    - **Cleanup**: Implemented `reset_db.py` for clean seeding cycles.
- **Result**: **Inventory Data Ready**. Verified via `verify_seed.py`.

---

## 📅 2026-03-09: Phase 22 - Auth Service Recovery & Dual Login ✅
**Goal**: Restore Auth Service functionality and implement support for RFID/Barcode login alongside standard email/password.

- **Service Recovery**:
    - **Fix**: Corrected `BaseEntity` import in `common/repository.py` (SSOT fix).
    - **Cleanup**: Eliminated redundant `database.py` in `auth_service/app`.
- **Dual Login Implementation**:
    - **Logic**: Bifurcated `/login` to handle `email/password` OR `identity_token`.
    - **Handshake**: Prioritized `X-Selection-Token` header in `get_selection_payload`.
    - **Schema**: Updated `LoginRequest` with optional fields for flexibility.
- **Audit Compliance**:
    - **Service**: 100% Compliance achieved via `AuditService.log_action` in Command Handlers.
- **Seeding v3.0**:
    - **Admin**: "Charly" user created with access to 2 companies.
    - **Operator**: "Operator01" created with RFID `RFID123456`.
- **Documentation**:
    - **Use Cases**: Detailed sequence diagrams in [auth_service_use_cases.md](file:///c:/API/interno/backend/auth_service/auth_service_use_cases.md).
- **Result**: **Service Online**. Dual login verified with 100% success.
