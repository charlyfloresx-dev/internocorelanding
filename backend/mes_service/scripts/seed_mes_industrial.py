...
# Adjustment of PYTHONPATH to find 'common' and 'app'
...
async def seed_mes_industrial():
    logger.info("🌱 Seeding MES Industrial Categories...")
    async with AsyncSessionLocal() as session:
        try:
            # 1. Seed Downtime Reasons
            ...
            logger.info("🚀 MES Industrial Seeding Completed.")
        except Exception as e:
            await session.rollback()
            logger.error(f"❌ Error in MES Industrial Seed: {e}")
            raise

if __name__ == "__main__":
    asyncio.run(seed_mes_industrial())
