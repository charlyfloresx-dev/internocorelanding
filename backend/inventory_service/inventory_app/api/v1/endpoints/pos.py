...
async def pos_checkout(
    ...
    # This prevents tampering and ensures Customer-specific pricing is enforced.
    ...
    for item in sale.items:
        ...
        # Layer 1: B2B Agreement (Highest Priority)
        if sale.customer_id:
        ...
        # Resolve which Price List index to use (Layer 2 & 3)
        target_list_index = 1
        ...
        # Layer 2: Warehouse-Specific Price (for the target list)
        ...
        # Layer 3: Global Company Price (for the target list)
        if resolved_price is None:
        ...
        # Layer 4: Fallback to List 1 (Public General)
        ...
        if resolved_price is not None:
            if abs(float(resolved_price) - item.unit_price) > 0.01:
                if not product.allow_price_override:
                    raise HTTPException(
                        status_code=400, 
                        detail=f"PRICE_ERR: Invalid price for {item.sku}. Expected {resolved_price}, got {item.unit_price}. Override not allowed."
                    )
        ...
        folio_id = str(correlation_id)[:5].upper()
        doc_entity = {
            "id": correlation_id,
            ...
            "external_reference": str(correlation_id)
        }
        ...
        import logging
        logging.error(f"POS_CHECKOUT_FAILED: {str(e)}", exc_info=True)
