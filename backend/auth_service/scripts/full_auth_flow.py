...
def run_full_flow():
    ...
    payload = {"company_id": target_company["company_id"]} # Usa el ID dinámico del login
    ...
    try:
        ...
        # --- PASO 3: DECODIFICACIÓN Y CONTRASTE ---
        print("\n🛡️ PASO 3: Verificación de Claims y Scopes (JWT vs JSON)...")
        ...
        print(f"   - Roles:  {data.get('roles', '⚠️ No enviado')}")
        print(f"   - Scopes: {data.get('scopes', '⚠️ No enviado')}")
        print("-" * 40)
