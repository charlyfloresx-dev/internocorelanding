...
async def run_test():
    async with httpx.AsyncClient(follow_redirects=True) as client:
        ...
        # 3. Fetch Warehouses to find a valid warehouse_id
        ...
        # 3.5 Fetch Concept ID
        print("Fetching concepts...")
        res_concepts = await client.get("http://localhost:8000/api/v1/concepts", headers=headers)
        if res_concepts.status_code != 200:
            print("Failed to fetch concepts:", res_concepts.text)
            return
        concepts = res_concepts.json()["data"]
        target_concept = next((c for c in concepts if c["code"] == "ENT-PUR"), None)
        if not target_concept:
            print("Concept ENT-PUR not found!")
            return
        concept_id = target_concept["id"]
        print(f"Using Concept ID: {concept_id}")
        ...
        doc_payload = {
            ...
            "notes": "Mobile Industrial Entry Forensic Test",
            "items": [
                {
                    "sku": "ECM-600",
