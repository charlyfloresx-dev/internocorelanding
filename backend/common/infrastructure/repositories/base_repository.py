...
class BaseRepository(Generic[T]):
    ...
    def _get_base_query(self):
        ...
        if context:
            # A. Aislamiento por Compañía
            if context.company_id and hasattr(self.model, "company_id"):
                query = query.where(self.model.company_id == context.company_id)
            
            # B. Geographic RBAC (Phase 33.5)
            # Si el usuario es un Operador, restringimos a sus almacenes permitidos.
            if context.role in ["OPERATOR", "WAREHOUSE_OPERATOR"] and \
               context.accessible_warehouses and \
               hasattr(self.model, "warehouse_id"):
                query = query.where(self.model.warehouse_id.in_(context.accessible_warehouses))
